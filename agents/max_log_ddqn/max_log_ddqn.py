...
class MaxLogDdqn(AgentBase):
    ...
    def reset(self, eval_stock_list=None):
        ...
        for stock in stock_list:
            ...
            self.ema_feature_sets[stock.ticker] = EmaFeatureSet(self.ema_windows)
    ...
    def take_action(self, eval_stock_list=None):
        ...
        for stock in stock_list:
            ...
            action = self.get_action_for_stock(stock)
            mean.append(feature_set.mean[action])
            var.append(feature_set.var[action])

        mean = np.array(mean)
        var = np.array(var)

        lambda_0 = 0
        lambdas = np.zeros(N) + lambda_0

        for _ in range(self.num_iter):
            M = 1 + (alpha * mean).sum()
            V = (alpha * alpha * var).sum()

            alpha = mean * ((M * M + V) - (lambda_0 + lambdas) * (M * M * M)) / (var * M)
            lambdas = (mean * (M * M + V) - alpha * var * M) / (M * M * M) - lambda_0
            lambda_0 = np.mean((mean * (M * M + V) - alpha * var * M) / (M * M * M) - lambdas)
    ...
    def get_action_for_stock(self, stock):
        feature_generator = self.feature_generators[stock.ticker]
        feature_set = self.ema_feature_sets[stock.ticker]
    ...
    def convert_action(self, agent, action, ticker, price):
        stock_list = agent.stock_list
        if eval_stock_list is not None:
            stock_list = eval_stock_list
        ...
        for stock in stock_list:
            price = stock.price()
            feature_set = agent.feature_sets[stock.ticker]
            feature_set.append_price(price)
